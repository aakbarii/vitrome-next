      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            observer.disconnect(); // بعد از اجرا، نظارت متوقف شود
          }
        });
      },
      { threshold: 0.5 } // 50٪ از المان دیده شود، اجرا شود
    );
            ...
            <div className="w-1/2 lg:w-1/4 px-4">
              <div className="flex flex-col sm:flex-row flex-wrap items-center gap-[15px]">
                <div className="text-[64px] leading-none font-bold flex flex-wrap items-center">
                  <Odometer value={counter.history} format="d" duration={2000} />
                </div>
                  <div className={`text ${theme === "light" ? "text-[#140c1c]" : ""}`}>سابقه <br /> کاری</div>
              </div>
            </div>
            <div className="w-1/2 lg:w-1/4 px-4">
              <div className="flex flex-col sm:flex-row flex-wrap items-center gap-[15px]">
                <div className="text-[64px] leading-none font-bold flex flex-wrap items-center">
                  <Odometer value={counter.completion} format="d" duration={2000} />+
                </div>
                  <div className={`text ${theme === "light" ? "text-[#140c1c]" : ""}`}>پروژه های <br /> تکمیل شده</div>
              </div>
            </div>
            <div className="w-1/2 lg:w-1/4 px-4">
              <div className="flex flex-col sm:flex-row flex-wrap items-center gap-[15px]">
                <div className="text-[64px] leading-none font-bold flex flex-wrap items-center">
                  <Odometer value={counter.satisfied} format="d" duration={2000} />K
                </div>
                  <div className={`text ${theme === "light" ? "text-[#140c1c]" : ""}`}>میزان <br /> رضایت</div>
              </div>
            </div>
            <div className="w-1/2 lg:w-1/4 px-4">
              <div className="flex flex-col sm:flex-row flex-wrap items-center gap-[15px]">
                <div className="text-[64px] leading-none font-bold flex flex-wrap items-center">
                  <Odometer value={counter.experience} format="d" duration={2000} />
                </div>
                  <div className={`text ${theme === "light" ? "text-[#140c1c]" : ""}`}>سال های <br /> تجربیات</div>
              </div>
            </div>
          </div>
        </div>
