        ...
        <div className="flex flex-wrap -mx-4">
          <div className="w-full lg:w-5/12 px-4">
            <div className="mb-[50px] max-w-[700px] w-full">
              <h2 data-aos="fade-left" className="text-[45px] bg-gradient-to-r from-[#8750f7] to-white bg-clip-text text-transparent inline-flex gap-[15px] mb-0">
              ...
              <p data-aos="fade-up" className={`mt-[15px] ${theme === "light" ? "text-[#140c1c]" : ""}`}>
                ...
                با داستان‌های مشتریان ما، به عمق تجربه‌های الهام‌بخش آن‌ها
                می‌پردازیم.
              </p>
            </div>
          </div>
                ...
                {comments?.map((comment, index) => (
                  <SwiperSlide key={index}>
                    <div className={`testimonial-item ${theme === "light" ? "bg-white" : "bg-[#140c1c]"} max-w-[300px] w-full rounded-[15px] p-[25px]`}>
                      ...
                      <p style={{ minHeight: "75px" }} className={`quote text-[16px] mb-[50px] rtl ${theme === "light" ? "text-[#140c1c]" : ""}`}>
                        {comment.paragraph}
                      </p>
                      <h4 className={`name text-[18px] mb-[5px] rtl ${theme === "light" ? "text-[#2a1454]" : "text-white"}`}>{comment.name}</h4>
                      <span className={`designation text-[14px] block rtl ${theme === "light" ? "text-[#140c1c]" : ""}`}>{comment.position}</span>
                    </div>
                  </SwiperSlide>
                ))}
              </Swiper>
