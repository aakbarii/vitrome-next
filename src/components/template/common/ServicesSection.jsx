          ...
          <div className="w-full px-4">
            <div className="relative" ref={serviceWidgetRef}>
              {services?.map((service, index) => (
                <div
                  key={index}
                  className={`service-item border-b ${
                    theme === "light" ? "border-[#8750f7]" : "border-[#2a1454]"
                  } px-[30px] relative z-[2] flex flex-wrap items-center gap-[20px] cursor-pointer group ${
                ...
                >
                  <div className="left-box w-[calc(40%-10px)] py-[30px] gap-[20px] flex flex-wrap items-center">
                    <span
                      className={`text-[20px] font-bold transition-all duration-500 ${
                        index === activeIndex
                          ? "text-white"
                          : "text-[#8750f7] group-hover:text-white"
                      }`}
                    >{`0${index + 1}`}</span>
                    <h3
                      className={`text-[30px] mb-0 capitalize ${
                  ...
                  <i
                    className={`absolute top-1/2 left-[55px] right-auto text-[20px] leading-none text-[#8750f7] transition-all duration-500 -translate-y-1/2 ${
                      index === activeIndex
                        ? "rotate-180 text-white"
                        : "rotate-180 group-hover:rotate-[270deg] group-hover:text-white"
                    }`}
                  ></i>
                </div>
              ))}
              <div
                className="active-bg absolute top-0 bottom-0 left-0 right-0 z-[1] bg-gradient-to-r from-[#2a1454] to-[#8750f7] transition-all duration-500"
                ref={activeBgRef}
              ></div>
            </div>
          </div>
